...
// News Content Management Component
function NewsContentManagement({
  newsContent,
  setNewsContent,
  showAddModal,
  setShowAddModal,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (thumbnailFile) {
        ...
        // Auto-generate thumbnail from YouTube URL if no custom thumbnail provided
        const videoIdMatch = formData.youtube_content.match(
          /(?:https?:\/\/)?(?:www\.)?(?:youtube\.com\/watch\?v=|youtu\.be\/)([a-zA-Z0-9_-]{11})/,
        );
        if (videoIdMatch && videoIdMatch[1]) {
      ...
      // For video content, store YouTube URL/embed code in the content field
      let contentToStore = formData.content;
      if (formData.type === "video" && formData.youtube_content) {
        // If there's YouTube content (URL or embed code), store it in the content field
        contentToStore = formData.youtube_content;
      }
      ...
      const newsData = {
        ...
        estimated_read_time: formData.estimated_read_time || null,
      ...
      let savedItem;
      if (editingItem) {
        // Update existing news item in Supabase
        ...
        // Create new news item in Supabase
        const { data, error } = await supabase
          .from("news")
          .insert(newsData)
          .select()
        ...
        // Update local state
        setNewsContent((prev) => [savedItem, ...prev]);
      }
      ...
      alert(
        `Failed to ${editingItem ? "update" : "create"} news item. Please try again.`,
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const handleEdit = (item) => {
    ...
    // For video items, extract YouTube content (URL or embed code) from content field
    let youtubeContent = "";
    let contentText = item.content || "";

    if (item.type === "video" && item.content) {
      // Check if content contains YouTube URL or embed code
      const youtubeUrlMatch = item.content.match(
        /(?:https?:\/\/)?(?:www\.)?(?:youtube\.com\/(?:watch\?v=|embed\/))|youtu\.be\//,
      );
      const embedCodeMatch = item.content.match(
        /<iframe[^>]*src=["']https:\/\/www\.youtube\.com\/embed\/[^"']*["'][^>]*><\/iframe>/i,
      );

      if (youtubeUrlMatch || embedCodeMatch) {
        youtubeContent = item.content;
        contentText = ""; // Clear content since it contains YouTube data
      }
    }
    ...
    setAutoGenerateThumbnail(!item.thumbnail); // If there's already a thumbnail, don't auto-generate
    setShowAddModal(true);
  };
  ...
  const handleDelete = async (id) => {
    if (confirm("Are you sure you want to delete this news item?")) {
      try {
        ...
        // Update local state
        setNewsContent((prev) => prev.filter((item) => item.id !== id));
  ...
  const handleSetAsFeaturedVideo = async (item) => {
    if (item.type === "video" && item.content) {
      try {
        // First, unset any existing featured videos
        await supabase
          .from("news")
        ...
        setNewsContent((prev) =>
          prev.map((newsItem) => ({
            ...newsItem,
            is_featured: newsItem.id === item.id && newsItem.type === "video",
          })),
        );

        // Extract YouTube URL and update featured video URL
        let videoUrl = null;

        // Extract YouTube URL from content field
        const youtubeUrlMatch = item.content.match(
          /(?:https?:\/\/)?(?:www\.)?(?:youtube\.com\/watch\?v=|youtu\.be\/)([a-zA-Z0-9_-]{11})/,
        );
        if (youtubeUrlMatch) {
          videoUrl = `https://www.youtube.com/watch?v=${youtubeUrlMatch[1]}`;
        }
        ...
        alert("Video set as featured video!");
      } catch (error) {
        console.error("Error setting featured video:", error);
        alert("Failed to set featured video. Please try again.");
      }
    } else {
      alert("Please ensure the content contains a valid YouTube URL");
    }
  };
      ...
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="flex items-center gap-2">
                <Newspaper className="h-5 w-5 text-[#032F60]" />
                News Content Management
              </CardTitle>
              <p className="text-sm text-gray-600 mt-1">
                Manage news articles, videos, and announcements
              </p>
            </div>
            <Button
              onClick={() => setShowAddModal(true)}
                      ...
                      <div className="flex items-center gap-4 text-xs text-gray-500">
                        ...
                        {item.type === "article" &&
                          item.estimated_read_time && (
                            <span>Read time: {item.estimated_read_time}</span>
                          )}
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-1 ml-4">
                      {item.type === "video" && (
                        <Button
                          variant="ghost"
                          size="sm"
                          onClick={() => handleSetAsFeaturedVideo(item)}
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              {editingItem
                ...
                : "Create a new news article or video for your team."}
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input
                  ...
                  placeholder="e.g., Market Updates, Company News"
                  required
                />
              </div>
              <div>
            ...
            {formData.type === "video" && (
              <div>
                <Label htmlFor="youtube_content">
                  YouTube URL or Embed Code
                </Label>
                <Textarea
                  id="youtube_content"
                  value={formData.youtube_content}
                  onChange={(e) =>
                    setFormData((prev) => ({
                      ...prev,
                      youtube_content: e.target.value,
                    }))
                  }
                  rows={4}
                  placeholder='Enter YouTube URL (e.g., https://www.youtube.com/watch?v=...) or full embed code (e.g., <iframe src="https://www.youtube.com/embed/..."></iframe>)'
                  required={formData.type === "video"}
                />
                <p className="text-xs text-gray-500 mt-1">
                  Supports: YouTube URLs, YouTube short URLs (youtu.be), and
                  full iframe embed codes
                </p>
              </div>
            )}
            ...
            <div>
              {formData.type === "video" ? (
                ...
                <div>
                  <Label htmlFor="estimated_read_time">
                    Estimated Read Time
                  </Label>
                  <Input
